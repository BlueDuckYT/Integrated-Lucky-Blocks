...
public class SpawnNearPlayerAction extends EventAction {
      ...
      public SpawnNearPlayerAction(String ent, int quant, int range) {
            entityResLoc = ResourceLocation.parse(ent);
            quantity = quant;
            dist = range;
      }
      public SpawnNearPlayerAction(String ent, int quant, int range, String name) {
            entityResLoc = ResourceLocation.parse(ent);
            quantity = quant;
            customName = name;
            dist = range;
      ...
      public void execute(ServerLevel level, BlockPos pos, Entity entity) {
            ...
            for (int i = 0; i < quantity; i++) {
                  ...
                  entToSpawn.moveTo(entity.getX() + 0.5 + level.getRandom().nextDouble() * dist * 2 - dist, entity.getY() + 0.5 + level.getRandom().nextDouble() * dist, entity.getZ() + 0.5 + level.getRandom().nextDouble() * dist * 2 - dist);
                  if (customName != null) {
